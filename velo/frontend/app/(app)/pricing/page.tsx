  ...
  return (
    <div>
      <div className="mb-10 text-center max-w-xl mx-auto">
        ...
        <p className="font-body text-sm text-slate-500">Comece grátis por 7 dias. Sem cartão de crédito.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 max-w-4xl mx-auto">
        {PLANS.map((plan) => (
          <div key={plan.id} className={`bg-white rounded-3xl border p-6 relative flex flex-col shadow-[0_1px_2px_rgba(15,25,35,0.04),0_16px_40px_-20px_rgba(15,25,35,0.12)] ${plan.id === "pro" ? "border-moss-300 ring-1 ring-moss-200" : "border-slate-200/80"}`}>
            {plan.tag && (
              <span className="absolute -top-3 left-1/2 -translate-x-1/2 font-body text-[10px] uppercase tracking-widest font-semibold bg-moss-600 text-white px-3 py-1 rounded-full">
                {plan.tag}
              </span>
            )}
            <div className="mb-5">
              <p className="font-body text-[10px] uppercase tracking-widest text-slate-400 font-semibold mb-1">{plan.name}</p>
              <p className="font-display font-black text-3xl text-slate-900">
                R${plan.price}<span className="font-body text-sm font-normal text-slate-400">/mês</span>
              </p>
              <p className="font-body text-xs text-slate-500 mt-1.5 leading-relaxed">{plan.description}</p>
            </div>
            ...
            {plan.current ? (
              <div className="text-center font-body text-xs text-slate-400 border border-slate-100 rounded-full py-2.5">
                {plan.cta}
              </div>
            ) : "href" in plan && plan.href ? (
              <a href={plan.href} className="block text-center font-body text-sm font-semibold text-slate-700 border border-slate-200 rounded-full py-2.5 hover:bg-slate-50 transition-colors">
                {plan.cta}
              </a>
            ) : (
              <button
                onClick={() => setModal(plan.id as "pro" | "agency")}
                className="block w-full text-center font-body text-sm font-semibold text-white bg-moss-600 rounded-full py-2.5 shadow-[0_8px_20px_-6px_rgba(63,107,78,0.4)] hover:bg-moss-700 transition-colors"
              >
                {plan.cta}
              </button>
            )}
          ...
          <div className="bg-white rounded-3xl border border-slate-200/80 shadow-[0_1px_2px_rgba(15,25,35,0.05),0_32px_64px_-24px_rgba(15,25,35,0.3)] p-8 w-full max-w-sm relative">
            ...
            <form onSubmit={handleInterest} className="space-y-3">
              <input
                ...
                required
                className="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 font-body text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-moss-600 bg-white"
              />
              <button type="submit" disabled={sending} className="w-full bg-moss-600 text-white py-2.5 rounded-full font-body text-sm font-semibold hover:bg-moss-700 transition-colors disabled:opacity-60">

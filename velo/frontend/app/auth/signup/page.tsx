  ...
  if (success) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-bone">
        <div className="bg-white border border-ink/10 p-8 rounded-xl text-center">
          <h2 className="font-display font-bold text-xl text-ink">Verifique seu e-mail</h2>
          <p className="mt-2 font-mono text-sm text-ink/50">Enviamos um link de confirmação para {email}.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-bone">
      <div className="w-full max-w-sm bg-white border border-ink/10 p-8 rounded-xl">
        ...
        <p className="font-mono text-xs text-ink/40 mb-6">Criar conta.</p>
        <form onSubmit={handleSignup} className="space-y-4">
          <div>
            <label className="block font-mono text-xs font-medium text-ink/60 mb-1">Nome da empresa</label>
            <input
              value={orgName}
        ...
        <p className="mt-4 font-mono text-xs text-center text-ink/40">
          Já tem conta?{" "}
          <a href="/auth/login" className="text-signal hover:underline">Entrar</a>
        </p>
      </div>
    </div>
  );
